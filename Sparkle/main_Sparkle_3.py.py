...
class MILP_SCHWAEMM():
    ...
    def SPLIT_fourth(self, m, X, S1, S2, S3, S4, size = None):
        if size == None:
            size = self.word_size
        for i in range(size):
            m.addConstr(X[i] >= S1[i])
            m.addConstr(X[i] >= S2[i])
            m.addConstr(X[i] >= S3[i])
            m.addConstr(X[i] >= S4[i])
            m.addConstr(S1[i] + S2[i] + S3[i] + S4[i] >= X[i])

    def XOR_fourth(self, m, U, H, L,Q, W, size = None):
        if size == None:
            size = self.word_size
        for i in range(size):
            m.addConstr( U[i] + H[i] + L[i] + Q[i] - W[i] == 0)
    ...
    def M4(self,m, CO_0, CO_1,CO_2,CO_3, MO_0,MO_1, MO_2,MO_3):
        x0 = CO_0[0:32]
        y0 = CO_0[32:64]
        x1 = CO_1[0:32]
        y1 = CO_1[32:64]
        ...
        x3_copy1 = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        x3_copy2 = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        ...
        y3_copy1 = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        y3_copy2 = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        ...
        Lx_copy4 = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        ...
        Ly_copy4 = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        ...
        self.SPLIT(m, x3, x3_copy1, x3_copy2, size=32)
        ...
        self.SPLIT(m, y3, y3_copy1, y3_copy2, size=32)

        self.XOR_fourth(m, x0_copy1, x1_copy1, x2_copy1,x3_copy1, Ox, size=32)
        self.XOR_fourth(m, y0_copy1, y1_copy1, y2_copy1,y3_copy1, Oy, size=32)
        ...
        self.SPLIT_fourth(m, Lx, Lx_copy1, Lx_copy2, Lx_copy3, Lx_copy4, size=32)
        self.SPLIT_fourth(m, Ly, Ly_copy1, Ly_copy2, Ly_copy3, Ly_copy4, size=32)
        ...
        self.XOR(m,Ly_copy4, x3_copy2,u3,size=32)

        self.XOR(m,Lx_copy1, y0_copy2,v0,size=32)
        self.XOR(m,Lx_copy2, y1_copy2,v1,size=32)
        self.XOR(m,Lx_copy3, y2_copy2,v2,size=32)
        self.XOR(m,Lx_copy4, y3_copy2,v3,size=32)



    def genModel512(self, R=4, input_X= None, output_X= None, count_num_solution = False, AEAD_mode = False ):
        m = Model()
        ...
        X = [[m.addVar(vtype=GRB.BINARY) for i in range(8 * self.word_size)] for r in range(R + 1)]
        ...
        ALZOUT = [[m.addVar(vtype=GRB.BINARY) for i in range(8 * self.word_size)] for r in range(R)]
        ...
        CO_3 = [[m.addVar(vtype=GRB.BINARY) for i in range(self.word_size)] for r in range(R)]
        ...
        MO_3 = [[m.addVar(vtype=GRB.BINARY) for i in range(self.word_size)] for r in range(R)]
        Y0_consta_LAST = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        Y1_consta_LAST = [m.addVar(vtype=GRB.BINARY) for i in range(32)]
        ALZOUT_LAST = [m.addVar(vtype=GRB.BINARY) for i in range(8 * self.word_size)]
        for i in range(8*self.word_size):
            m.addConstr(X[0][i] == input_X[i])
        ...
        for r in range(R):
            ...
            self.AlzetteSbox(m, constant_values[6], X[r][384:448],ALZOUT[r][384:448], R_az=4)
            self.AlzetteSbox(m, constant_values[7], X[r][448:512],ALZOUT[r][448:512], R_az=4)


            self.SPLIT(m, ALZOUT[r][0:64], CO_0[r], X[r+1][256:320], size=64) #(x_0,y_0)
            self.SPLIT(m, ALZOUT[r][64:128], CO_1[r], X[r+1][320:384], size=64)  # (x_1,y_1)
            self.SPLIT(m, ALZOUT[r][128:192], CO_2[r], X[r+1][384:448], size=64)  # (x_2,y_2)
            self.SPLIT(m, ALZOUT[r][192:256], CO_3[r], X[r+1][448:512], size=64)  # (x_3,y_3)

            self.M4(m,CO_0[r],CO_1[r], CO_2[r],CO_3[r], MO_0[r],MO_1[r], MO_2[r], MO_3[r])
            self.XOR(m,MO_0[r],ALZOUT[r][256:320], X[r+1][192:256], size = 64)
            self.XOR(m,MO_1[r],ALZOUT[r][320:384], X[r+1][0:64], size = 64)
            self.XOR(m,MO_2[r],ALZOUT[r][384:448], X[r+1][64:128], size = 64)
            self.XOR(m,MO_3[r],ALZOUT[r][448:512], X[r+1][128:192], size = 64)
        ...
        self.AlzetteSbox(m, constant_values[6], X[R][384:448], ALZOUT_LAST[384:448], R_az=4)
        self.AlzetteSbox(m, constant_values[7], X[R][448:512], ALZOUT_LAST[448:512], R_az=4)
...
if __name__ == "__main__":
    ...
    if sparkle512 == True:
        print("------genModel512------")
        start_time = time.time()
        for index_output_bit in range(0, 512): #127 190 #254
            input_X = ( [0] * 64 ) + ( [0] * 64 ) + ( [0] * 63 )+[0] + ( [1] * 64 ) + [ 1 for index_j_1 in range(256, 512) ] #128
            output_X = [0 for i in range(512)]
            output_X[index_output_bit] = 1

            res = milp.genModel512(R=Rd, input_X= input_X, output_X= output_X, count_num_solution = False)
