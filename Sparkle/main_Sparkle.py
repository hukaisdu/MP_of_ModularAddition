...
class MILP_SCHWAEMM():
    ...
    def M2(self,m, CO_0, CO_1, MO_0,MO_1):
        x0 = CO_0[0:32]
        y0 = CO_0[32:64]
        x1 = CO_1[0:32]
        y1 = CO_1[32:64]
    ...
    def genModel256(self, R=4, input_X= None, output_X= None, count_num_solution = False, AEAD_mode = False ):
        m = Model()
        ...
        X = [[m.addVar(vtype=GRB.BINARY) for i in range(4 * self.word_size)] for r in range(R + 1)]
        ...
        ALZOUT = [[m.addVar(vtype=GRB.BINARY) for i in range(4 * self.word_size)] for r in range(R)]
        ...
        ALZOUT_LAST = [m.addVar(vtype=GRB.BINARY) for i in range(4 * self.word_size)]
        for i in range(4*self.word_size):
            m.addConstr(X[0][i] == input_X[i])
            # m.addConstr(X[R][i] == output_X[i])
        ...
        for r in range(R):
            ...
            self.SPLIT(m, ALZOUT[r][0:64], CO_0[r], X[r+1][128:192], size=64) #(x_0,y_0)
            self.SPLIT(m, ALZOUT[r][64:128], CO_1[r], X[r+1][192:256], size=64)  # (x_1,y_1)

            self.M2(m,CO_0[r],CO_1[r],MO_0[r],MO_1[r])
            self.XOR(m,MO_0[r],ALZOUT[r][128:192], X[r+1][64:128], size = 64)
            self.XOR(m,MO_1[r],ALZOUT[r][192:256], X[r+1][0:64], size = 64)
...
if __name__ == "__main__":
    ...
    if sparkle256 == True:
        print("------Sparkle256------")
        start_time = time.time()
        for index_output_bit in range(0, 256): #127
            input_X = [0 for index_j_2 in range(0, 64)]+ ( [1] * 64 ) + [ 1 for index_j_1 in range(0, 128) ] #128
            output_X = [0 for i in range(256)]
            output_X[index_output_bit] = 1

            res = milp.genModel256(R=Rd, input_X= input_X, output_X= output_X, count_num_solution = False)
